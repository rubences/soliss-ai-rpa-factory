...
(() => {
  ...
  function render(){
    ...
    const E=T.economics;
    $('#tenderSummaryCards').innerHTML=[['Objeto','P0 · infraestructura crítica y plataforma IA/RPA'],['Alcance contractual','P0 + servicios derivados'],['Duración','Según condiciones aprobadas'],['Servicios Keedio','Reservado'],['Infra Soliss','Pendiente de sizing'],['UC1–UC7','Derivados · partidas adicionales']].map(x=>`<article><span>${x[0]}</span><b>${x[1]}</b></article>`).join('');
    $('#tenderReconciliation').innerHTML='<div><span>CONTROL DE COHERENCIA ECONÓMICA</span><h3>Reconciliar condiciones antes de publicar el pliego.</h3><p>El detalle económico y la distribución de dedicación quedan reservados para documentación autorizada. Esta vista muestra únicamente el estado de revisión y sus dependencias.</p></div><strong>REQUIERE DECISIÓN</strong>';
    ...
    $('#economicSummary').innerHTML=[['Construcción P0','Reservado','Servicios Keedio'],['Servicio co-gestionado','Reservado','Según contrato aprobado'],['Total contractual','Reservado','No publicado'],['Infraestructura Soliss','Pendiente de sizing','Compra directa'],['TCO orientativo','No publicado','Requiere aprobación'],['Opcionales','Según condiciones aprobadas','Sujeto a decisión']].map(x=>`<article><span>${x[0]}</span><b>${x[1]}</b><small>${x[2]}</small></article>`).join('');
    $('#tenderPhaseTable').innerHTML=`<thead><tr><th>Bloque</th><th>Periodo</th><th>Dedicación</th><th>Importe</th><th>Tarifa</th><th>Objeto</th></tr></thead><tbody>${E.phases.map(x=>`<tr><td><b>${safe(x[0])}</b></td><td>Según planificación aprobada</td><td>Reservado</td><td>Reservado</td><td>Reservado</td><td>${safe(x[5])}</td></tr>`).join('')}</tbody>`;
