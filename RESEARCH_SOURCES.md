# Research Radar 2026 · Keedio → Soliss

Actualizado: 1 septiembre 2026. Material de contexto para propuesta; no sustituye revisión Legal/Compliance.

## Hallazgos incorporados en la Decision Room
- EIOPA mantiene DORA como Focus Area supervisora 2026 y pone foco en board engagement, ICT framework, incidentes, terceros y resilience testing.
- Claims management, incluida su digitalización, es Area for Attention 2026; EIOPA cita 62% de satisfacción UE con claims handling.
- EIOPA Opinion sobre AI governance: data governance, record-keeping, fairness, cyber security, explainability y human oversight.
- AI Act: transparencia Art. 50 aplicable desde agosto de 2026; chatbots cubiertos deben informar interacción con IA.
- AI literacy: Article 4 aplica desde febrero 2025; la organización debe sostener medidas adaptadas a roles/contexto.
- Annex III 5(c): risk assessment y pricing de personas en vida/salud; claims management no entra automáticamente en ese supuesto.
- ISO/IEC 42001 se usa solo como alineamiento de sistema de gestión; no se afirma certificación.
- OWASP GenAI/Agentic y NIST AI RMF se usan como referencias técnicas voluntarias de risk engineering.

## Fuentes públicas
- https://www.soliss.es/
- https://www.eiopa.europa.eu/publications/generative-ai-market-survey-outlook-use-cases-and-risk-management_en
- https://www.eiopa.europa.eu/publications/opinion-artificial-intelligence-governance-and-risk-management_en
- https://www.eiopa.europa.eu/publications/union-wide-strategic-supervisory-priorities-focus-areas-2026_en
- https://digital-strategy.ec.europa.eu/en/policies/enforcement-ai-act
- https://digital-strategy.ec.europa.eu/en/policies/ai-talent-skills-and-literacy
- https://ai-act-service-desk.ec.europa.eu/en/essential-services
- https://www.iso.org/standard/42001
- https://genai.owasp.org/llm-top-10/
- https://airc.nist.gov/technical-reports/


## V6 verification refresh · 2 Sep 2026

Official sources re-checked for V6 Source Freshness:

- EIOPA — Generative AI Market Survey (65% adoption; 64% use cases in PoC/experimentation; 32% in production; 49% dedicated AI policy).
- EIOPA — Union-wide Supervisory Priorities 2026 (DORA Focus Area; claims management including digitalisation Area for Attention).
- EIOPA — Eurobarometer 2025 consumer trends (62% claims-handling satisfaction; 30% trust AI-agent recommendations for insurance/pensions).
- European Commission — Article 50 transparency guidelines (apply from 2 Aug 2026).
- European Commission — AI Act implementation timeline after AI Omnibus (Annex III 2 Dec 2027; Annex I 2 Aug 2028).
- ISO — ISO/IEC 42001:2023 AIMS.
- OWASP GenAI — LLM01 Prompt Injection and LLM06 Excessive Agency.
- Keycloak — public SPA clients, Standard Flow and PKCE S256.
- oauth2-proxy — verified JWT bearer-token support via `skip_jwt_bearer_tokens`.

The web records the verification date but does not treat external sources as contractual Soliss evidence.
